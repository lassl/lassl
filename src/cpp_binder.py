...
_datasets_utils_compiling_success = None
...
def get_datasets_utils():
    global _datasets_utils, _datasets_utils_compiling_success

    if _datasets_utils_compiling_success is None:
        try:
            _datasets_utils = Binder().bind()
            _datasets_utils_compiling_success = True
            return get_datasets_utils()
        except Exception as e:
            print(f"Failed to comiple C++ dataset utils... using slower python version.\nError message: {e}")
            _datasets_utils_compiling_success = False
            return get_datasets_utils()

    elif _datasets_utils_compiling_success is True:
        assert _datasets_utils is not None, "C++ dataset utils must not be None."
        return _datasets_utils
